package net.crazysnailboy.mods.skeletonhorses.client.model;
...
public class ModelHorse extends ModelBase
{
	...
	public void render(Entity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scale)
	{
		...
		boolean canHaveChest = true; // abstracthorse instanceof AbstractChestHorse;

		boolean hasChest = !isChild && canHaveChest && ((Supplier<Boolean>)(() -> {

			if (abstracthorse.hasCapability(CapabilityHorseChestHandler.HORSE_CHEST_CAPABILITY, null))
			{
				IHorseChestHandler capability = abstracthorse.getCapability(CapabilityHorseChestHandler.HORSE_CHEST_CAPABILITY, null);
				return capability.hasChest();
			}
			return false;

		})).get();
//		boolean hasChest = !isChild && canHaveChest && ((AbstractChestHorse)abstracthorse).hasChest();
	...
	private float updateHorseRotation(float prevRenderYaw, float renderYaw, float partialTicks)
	{
		float f;

		for (f = renderYaw - prevRenderYaw; f < -180.0F; f += 360.0F)
		{
			;
		}
		...
		return prevRenderYaw + partialTicks * f;
	}


	@Override
	public void setLivingAnimations(EntityLivingBase entitylivingbase, float limbSwing, float limbSwingAmount, float partialTicks)
	{
		super.setLivingAnimations(entitylivingbase, limbSwing, limbSwingAmount, partialTicks);
		float f = this.updateHorseRotation(entitylivingbase.prevRenderYawOffset, entitylivingbase.renderYawOffset, partialTicks);
		float f1 = this.updateHorseRotation(entitylivingbase.prevRotationYawHead, entitylivingbase.rotationYawHead, partialTicks);
		float f2 = entitylivingbase.prevRotationPitch + (entitylivingbase.rotationPitch - entitylivingbase.prevRotationPitch) * partialTicks;
		float f3 = f1 - f;
		...
		float f5 = abstracthorse.getGrassEatingAmount(partialTicks);
		float f6 = abstracthorse.getRearingAmount(partialTicks);
		float f7 = 1.0F - f6;
		float f8 = abstracthorse.getMouthOpennessAngle(partialTicks);
		...
		float f9 = (float)entitylivingbase.ticksExisted + partialTicks;
